...
# 【PTrade适配要点】
# 1. 股票代码：上海.SS，深圳.SZ
# 2. 价格获取：data[etf].price（交易时）+ 盘前预存复权因子
# 3. 策略因子：get_history（盘前/动量计算时）
# 4. 拆单处理：卖出按股数(90万股/笔)，买入按金额(order_value)
# 5. 涨停检查：涨停时跳过买入
# 6. 资金上限：g.capital_ratio 控制策略可用资金比例
#
# ==============================
