# 小市值波段策略 v5.3 - PTrade版本（修复组合跌幅检查的资金配比问题）
#
...
def handle_data(context, data):
    ...
    - 13:30 冷静期检查
    - 14:30 冷静期检查
    - 14:49 卖出（sell_stocks）
    - 其他时间：分钟风控（interval_sell_buy）
...
def buy_stocks(context, data):
    ...
    log.info("[09:31] 买入: 总资产%.2f, 策略资金%.2f(%.0f%%), 每只%.2f" 
             % (total_value, strategy_capital, g.capital_ratio * 100, per_value))
    log.info("[09:31] 已持仓%d, 需买入%d" % (len(held), need_num))
    ...
    if bought_count > 0:
        log.info("[09:31] 完成: 买入%d只" % bought_count)
