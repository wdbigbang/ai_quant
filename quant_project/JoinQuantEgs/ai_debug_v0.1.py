...
重构原则：
1. 单文件约束：严格在单个.py文件中完成重构
2. 原型信任：原型策略代码信任度高，不擅自魔改核心逻辑
3. 参数保留：所有可调参数全部保留，便于后续优化
4. 扩展优先：充分考虑未来增删改策略的可扩展性
5. 实盘贴近：交易设置贴近实盘（佣金、印花税、滑点等）

代码瘦身说明（v8.4.0）：
- 删除16个未使用的函数（技术指标、评分、批量交易类）
- 所有删除的函数都使用未来函数，不适合实盘交易
- SwingStrategy和ETFRotationStrategy运行不受影响
- 代码更加精简，适合实盘交易使用
...
def _init_global_variables(context):
    ...
    # ========== 全仓模式配置（用于测试单一策略效果） ==========
    g.enable_full_ratio_mode = True  # 是否开启全仓模式（默认关闭）
    ...
    if g.test_mode and g.strategy_registration.get('test_2', False):
        # 创建ETF核心资产轮动策略实例（开启自动获取ETF池，使用更严格的筛选条件）
        etf_params = {
            'auto_fetch_etf_pool': True,  # 开启自动获取ETF池
            'auto_fetch_num': 10,  # 目标获取10只ETF
            'auto_fetch_min_days': 480,  # 最小上市天数480天（约2年，平衡流动性和历史数据）
        }
    ...
    # ========== 【阶段六：Swing策略实装完成】 ==========
    log.info("=" * 60)
    log.info("[阶段六] SwingStrategy正式实装完成")
    log.info("=" * 60)
    log.info("  实装说明：")
    log.info("  - test_strategy_1已使用SwingStrategy实装")
    log.info("  - 所有测试代码已注释")
    log.info("  - execute_trade方法中的测试调用已移除")
    log.info("  - Swing策略的真正交易逻辑已启用")
    log.info("  - 定时任务正常调用：")
    log.info("    * swing_strategy_buy (09:31): 买入逻辑")
    log.info("    * swing_strategy_sell (14:49): 卖出逻辑")
    log.info("    * swing_strategy_interval_control (每分钟): 分钟级风控")
    log.info("    * swing_strategy_portfolio_monitor (09:30): 组合跌幅监控")
    log.info("=" * 60)
    # ========== 【阶段六：Swing策略实装完成】结束 ==========
...
def _check_cooldown_status(context):
    ...
    try:
        ...
        if g.test_mode and g.strategy_registration.get('test_1', False) and hasattr(g, 'test_strategy_1'):
            ...
            # 打印冷静期状态
            if in_cooldown:
                log.info(f"[冷静期状态] {strategy.strategy_id} 当前处于冷静期")
                log.info(f"  已过天数: {days_since_sell}/{cooldown_days} 天")
                log.info(f"  最后卖出日期: {last_sell_date if last_sell_date else '无'}")
            else:
                log.debug(f"[冷静期状态] {strategy.strategy_id} 未处于冷静期")
...
def _check_capital_pool_adjustment(context):
    ...
    try:
        ...
        # 检查是否为新的月份（即每月的第一个交易日）
        if current_month != g.last_adjustment_month:
            log.info(f"今日为本月({current_month}月)第一个交易日，检查资金池调整...")
            ...
            if hasattr(g, 'strategy_manager') and hasattr(g.strategy_manager, 'capital_pools'):
                ...
                log.info(f"当前资金池状态（非调整日，本月{current_day}号）:")
...
def transform_date(date_input: Any, output_format: str = 'str_yyyy_mm_dd') -> Optional[Union[str, dt.datetime, dt.date, int]]:
    ...
    支持多种日期格式输入，转换为指定格式输出
    
    参数:
        date_input: 输入日期，支持以下格式：
            - datetime.datetime 对象
            - datetime.date 对象
            - 字符串格式（如 '2026-02-02', '20260202', '2026/02/02'）
            - 时间戳（int 或 float）
            - numpy.datetime64 对象
            - pandas.Timestamp 对象
        output_format: 输出格式，支持：
            - 'str_yyyy_mm_dd': 标准字符串格式 'YYYY-MM-DD'（默认）
            - 'str_yyyymmdd': 聚宽日期格式 'YYYYMMDD'
            - 'datetime': datetime.datetime 对象
            - 'date': datetime.date 对象
            - 'timestamp': Unix 时间戳（秒）
    
    返回:
        根据output_format参数返回对应格式的日期
        如果转换失败，返回 None
    
    示例:
        >>> transform_date('2026-02-02', 'str_yyyymmdd')
        '20260202'
        >>> transform_date(datetime(2026, 2, 2), 'date')
...
def get_trading_time_status(context: Any, time_input: Optional[Union[dt.time, str]] = None) -> Optional[str]:
    ...
    判断给定时间是否在交易时段内，返回交易时段标识
    
    实盘移植说明：
    - QMT/PTrade平台：需要根据各平台的交易时段配置进行调整
    - 时间获取方式：context.current_dt 在不同平台可能有不同的格式
    - 节假日处理：需要考虑各平台的节假日安排
    
    参数:
        context: 策略上下文对象
        time_input: 输入时间，支持以下格式：
            - None: 使用 context.current_dt
            - datetime.time 对象
            - 字符串格式（如 '09:30', '09:30:00'）
    
    返回:
        str: 交易时段标识，可能的值：
            - 'pre_market': 盘前集合竞价（09:25-09:30）
            - 'morning': 上午交易时段（09:30-11:30）
            - 'lunch_break': 午休时段（11:30-13:00）
            - 'afternoon': 下午交易时段（13:00-15:00）
            - 'after_market': 盘后时段（15:00以后）
            - 'non_trading': 非交易时段（00:00-09:25）
            - None: 时间格式无效或无法识别
    """
    try:
        # 定义交易时段（转换为分钟数，便于比较）
...
def get_all_etf_list(date=None):
    """
    获取所有ETF基金列表（优化版）
    ...
    - 筛选出ETF基金（根据名称或代码特征）
    - 过滤掉不活跃的ETF
    - 提供备选方案：如果自动获取失败，使用预定义的优质ETF池
    
    参数:
        date: 查询日期，默认为None（使用当前日期）
    
    返回:
        DataFrame: ETF基金列表，包含代码、名称、类型等信息
    
    筛选条件：
    - 证券类型为'fund'
    - 名称包含'ETF'或代码以'51'或'159'开头（ETF常见代码）
    - 排除已退市或停牌的ETF
    """
    try:
        log.info(f"【ETF池】开始获取所有基金列表...")
        
        # 获取所有基金
        all_funds = get_all_securities(types=['fund'], date=date)
        
        log.info(f"【ETF池】获取到基金数量: {len(all_funds) if all_funds is not None else 0}")
        
        if all_funds is None or all_funds.empty:
            log.warning("【ETF池】未能获取基金列表（get_all_securities返回空），使用预定义优质ETF池")
            return _get_predefined_etf_list()
        ...
        for code, row in all_funds.iterrows():
            try:
                # 获取基金详细信息（使用get_security_info获取正确的名称）
                info = get_security_info(code)
                if info is None:
                    continue
                
                # 使用info.display_name获取对阅读友好的中文名称
                # 如果display_name不存在，则使用name作为备选
                fund_name = info.display_name if hasattr(info, 'display_name') else (info.name if hasattr(info, 'name') else row.get('name', ''))
                
                # 筛选条件1：名称包含'ETF'
                if 'ETF' not in fund_name:
                    continue
                ...
                # 筛选条件3：排除LOF基金（ETF和LOF都是基金类型）
                # ETF通常以'51'（上海）或'159'（深圳）开头
                if not (code.startswith('51') or code.startswith('159')):
                    continue
                ...
                etf_list.append({
                    'code': code,
                    'name': fund_name,
                    'type': info.type if hasattr(info, 'type') else 'fund',
        ...
        if not etf_df.empty:
            log.info(f"【ETF池】筛选出 {len(etf_df)} 只符合条件的ETF基金")
        else:
            log.warning("【ETF池】未找到符合条件的ETF基金，使用预定义优质ETF池")
            return _get_predefined_etf_list()
        
        return etf_df
        
    except Exception as e:
        log.error(f"【ETF池】获取ETF基金列表失败: {str(e)}，使用预定义优质ETF池")
        import traceback
        log.error(traceback.format_exc())
        return _get_predefined_etf_list()


def _get_predefined_etf_list():
    """
    获取预定义的优质ETF池（备选方案）
    
    当自动获取ETF失败时，使用这个预定义的优质ETF池
    包含主流宽基ETF和优质行业ETF，避免周期性行业
    流动性要求：日均成交量 >= 1000万（严格筛选）
    
    返回:
        DataFrame: 预定义的ETF基金列表
    """
    try:
        # 预定义的优质ETF池（经过优化筛选，日均成交量 >= 1000万）
        predefined_etfs = [
            # 宽基指数ETF（优先级最高，流动性好）
            {'code': '510300.XSHG', 'name': '沪深300ETF', 'type': 'fund', 'start_date': dt.date(2012, 5, 28)},
            {'code': '510500.XSHG', 'name': '中证500ETF', 'type': 'fund', 'start_date': dt.date(2013, 3, 15)},
            {'code': '159915.XSHE', 'name': '创业板ETF', 'type': 'fund', 'start_date': dt.date(2011, 9, 20)},
            {'code': '510050.XSHG', 'name': '上证50ETF', 'type': 'fund', 'start_date': dt.date(2004, 12, 30)},
            {'code': '588000.XSHG', 'name': '科创50ETF', 'type': 'fund', 'start_date': dt.date(2020, 7, 22)},
            {'code': '512100.XSHG', 'name': '中证1000ETF', 'type': 'fund', 'start_date': dt.date(2016, 9, 29)},
            
            # 科技成长ETF（流动性好，高流动性要求）
            {'code': '515000.XSHG', 'name': '科技ETF', 'type': 'fund', 'start_date': dt.date(2019, 7, 22)},
            {'code': '515760.XSHG', 'name': '芯片ETF', 'type': 'fund', 'start_date': dt.date(2020, 1, 20)},
            {'code': '159861.XSHE', 'name': '半导体ETF', 'type': 'fund', 'start_date': dt.date(2020, 5, 26)},
            
            # 医药消费ETF（流动性好）
            {'code': '512010.XSHG', 'name': '医药ETF', 'type': 'fund', 'start_date': dt.date(2015, 5, 20)},
            {'code': '159928.XSHE', 'name': '消费ETF', 'type': 'fund', 'start_date': dt.date(2013, 9, 23)},
            
            # 商品ETF（黄金，避险资产）
            {'code': '518880.XSHG', 'name': '黄金ETF', 'type': 'fund', 'start_date': dt.date(2013, 7, 18)},
        ]
        
        etf_df = pd.DataFrame(predefined_etfs)
        log.info(f"【ETF池】使用预定义优质ETF池（日均成交量 >= 1000万），共 {len(etf_df)} 只ETF")
        
        # 输出ETF列表
        for i, etf in enumerate(predefined_etfs):
            log.info(f"  {i+1}. {etf['code']} - {etf['name']}")
        
        return etf_df
        
    except Exception as e:
        log.error(f"获取预定义ETF池失败: {str(e)}")
        return pd.DataFrame()
        import traceback
        log.error(traceback.format_exc())
        return pd.DataFrame()
...
def get_etf_pool_by_criteria(context=None, target_num=10, min_days_listed=480):
    """
    根据筛选条件获取ETF池（优化版）
    
    功能说明：
    - 获取所有ETF基金
    - 根据严格的流动性、类型等指标筛选优质ETF
    - 排除周期性行业ETF和受外部因素影响大的ETF
    - 返回指定数量的ETF列表
    
    参数:
        context: 策略上下文对象（可选）
        target_num: 目标ETF数量（默认10只）
        min_days_listed: 最小上市天数（默认480天，约2年，确保足够历史数据）
    
    返回:
        List[str]: ETF代码列表
    
    优化后的筛选条件：
    1. 上市天数 >= 480天（约2年，确保足够历史数据）
    2. 流动性要求：平均成交量 >= 1000万股（提高门槛，更严格）
    3. 类型优化：
       - 优先选择：主流宽基ETF（沪深300、中证500、创业板、上证50、中证1000等）
       - 优先选择：成熟稳定行业ETF（科技、医药、消费等）
       - 排除：周期性行业ETF（光伏、稀有金属、钢铁、有色、电力等）
       - 排除：受外部因素影响大的ETF（中概互联等）
       - 排除：小众主题ETF（军工、环保等）
    """
    try:
        # 获取所有ETF基金
        etf_df = get_all_etf_list()
        
        if etf_df is None or etf_df.empty:
            log.warning("【ETF池】未获取到ETF基金列表")
            return []
        
        # 获取参考日期（盘前数据获取必须使用context.previous_date）
        if context:
            ref_date = context.previous_date
        else:
            ref_date = dt.date.today()
        ...
        # 定义需要排除的ETF类型（周期性行业、受外部因素影响大的行业）
        excluded_keywords = [
            # 周期性行业
            '光伏', '太阳能', '风电', '核电', '电力',
            '稀有金属', '钢铁', '有色', '煤炭', '石油', '原油',
        ...
        # ========== 筛选条件1：上市天数筛选 ==========
        valid_etfs = []
        log.info(f"【ETF池】开始上市天数筛选，待筛选ETF数量: {len(etf_df)}")
        ...
        for _, row in etf_df.iterrows():
            try:
                ...
                if matched_keywords:
                    # 因排除关键词被过滤，不输出DEBUG日志
                    continue
                
                # 删除调试信息：排除关键词检查已正常工作
                
                # 转换start_date为datetime.date对象（确保类型正确）
                start_date_converted = start_date
                if isinstance(start_date, str):
                    start_date_converted = dt.datetime.strptime(start_date, '%Y-%m-%d').date()
                elif isinstance(start_date, dt.datetime):
                    start_date_converted = start_date.date()
                elif not isinstance(start_date, dt.date):
                    # 如果不是预期的类型，跳过
                    log.warning(f"【ETF池】{code} {name}: start_date类型不正确 ({type(start_date)}), 值={start_date}, 跳过")
                    continue
                
                # 计算上市天数
                days_listed = (ref_date - start_date_converted).days
                
                # 检查days_listed是否为负数（异常情况）
                if days_listed < 0:
                    log.warning(f"【ETF池】{code} {name}: 上市天数为负数 ({days_listed}天), 上市日期可能在参考日期之后，跳过")
                    continue
                
                # 筛选：上市天数 >= min_days_listed
                ...
                log.error(f"【ETF池】处理ETF失败: {row.get('code', 'unknown')} - {str(e)}")
                import traceback
                log.error(traceback.format_exc())
                continue
        
        if not valid_etfs:
            log.warning(f"【ETF池】没有上市天数 >= {min_days_listed} 天的ETF")
            return []
        
        log.info(f"【ETF池】上市天数筛选后剩余 {len(valid_etfs)} 只ETF")
        
        # ========== 筛选条件2：流动性筛选（提高到1000万股，更严格） ==========
        liquid_etfs = []
        empty_volume_count = 0
        low_volume_count = 0
        exception_count = 0
        
        # 调试信息：输出参考日期和前几只ETF的代码
        log.info(f"【ETF池】开始流动性筛选（参考日期: {ref_date}, 获取天数: 20, 流动性要求: >=1000万股）")
        ...
        for etf in valid_etfs:
            try:
                ...
                # 计算平均成交量（最近20天），单位：万股
                avg_volume = volume_data['volume'].mean()  # 股数
                avg_volume_wan = avg_volume / 10000  # 转换为万股

                # 筛选：平均成交量 >= 1000万股（提高流动性要求，更严格）
                if avg_volume_wan >= 1000:
                    liquid_etfs.append({
                        'code': code,
                        'name': etf['name'],
                        'avg_volume': avg_volume_wan  # 保存为万股
                    })
                else:
                    low_volume_count += 1
            except Exception as e:
        ...
        # 输出流动性筛选的统计信息
        log.info(f"【ETF池】流动性筛选统计: 总数={len(valid_etfs)}, 无成交量={empty_volume_count}, 成交量不足={low_volume_count}, 通过筛选={len(liquid_etfs)}, 异常={exception_count}")
        
        if not liquid_etfs:
            log.warning("【ETF池】没有通过流动性筛选的ETF")
            # 降级：返回上市天数筛选的结果，并添加avg_volume字段
            liquid_etfs = []
            for etf in valid_etfs:
                liquid_etfs.append({
                    'code': etf['code'],
                    'name': etf['name'],
                    'avg_volume': 0  # 降级处理，成交量设为0万股
                })
        
        log.info(f"【ETF池】流动性筛选后剩余 {len(liquid_etfs)} 只ETF")
        
        # ========== 筛选条件2.5：规模筛选（新增） ==========
        scale_etfs = []
        no_fundamental_count = 0
        low_scale_count = 0
        exception_count = 0
        
        log.info(f"【ETF池】开始规模筛选（最小规模: 10亿元）")
        ...
        for etf in liquid_etfs:
            try:
                ...
                df = get_fundamentals(q, ref_date)
                
                if df is None or df.empty:
                    no_fundamental_count += 1
                    # 无法获取规模数据，保留该ETF（降级处理）
                    scale_etfs.append({
                        'code': code,
                        'name': etf['name'],
                        'avg_volume': etf['avg_volume'],
                        'market_cap': 0  # 规模未知，设为0
                    })
                    continue
                
                # 获取ETF规模（单位：亿元）
                market_cap = df['market_cap'].iloc[0]  # 亿元
                
                # 筛选：规模 >= 10亿元（排除规模过小的ETF）
                if market_cap >= 10:
                    scale_etfs.append({
                        'code': code,
                        'name': etf['name'],
        ...
        # 输出规模筛选的统计信息
        log.info(f"【ETF池】规模筛选统计: 总数={len(liquid_etfs)}, 无基本面数据={no_fundamental_count}, 规模不足={low_scale_count}, 通过筛选={len(scale_etfs)}, 异常={exception_count}")
        
        # 更新liquid_etfs为scale_etfs
        liquid_etfs = scale_etfs
        
        log.info(f"【ETF池】规模筛选后剩余 {len(liquid_etfs)} 只ETF")
        
        # ========== 筛选条件3：类型优化筛选 ==========
        # 按ETF类型分类，确保多样性并优先选择优质类型
        etf_types = {
            '宽基指数': [],      # 沪深300、中证500、创业板、上证50、中证1000等
            '科技成长': [],      # 科技、新能源（非光伏）等
            '医药消费': [],      # 医药、消费等
            '金融地产': [],      # 金融、地产等（可选）
            '商品': []          # 黄金等商品
        ...
        for etf in liquid_etfs:
            ...
            # 分类逻辑（根据名称关键词）
            if any(keyword in name for keyword in ['沪深300', '中证500', '创业板', '上证50', '中证1000']):
                etf_types['宽基指数'].append(code)
            elif any(keyword in name for keyword in ['科技', '半导体', '芯片', '人工智能', 'AI', '5G']):
                etf_types['科技成长'].append(code)
            elif any(keyword in name for keyword in ['医药', '生物', '医疗', '消费', '白酒']):
                etf_types['医药消费'].append(code)
            elif any(keyword in name for keyword in ['金融', '证券', '银行']):
                etf_types['金融地产'].append(code)
            elif any(keyword in name for keyword in ['黄金', '白银']):
                etf_types['商品'].append(code)
            else:
                # 其他类型归为宽基指数（优先选择宽基）
                etf_types['宽基指数'].append(code)
        ...
        # 优先级1：宽基指数ETF（最多4只）
        selected_etfs.extend(etf_types['宽基指数'][:4])
        
        # 优先级2：科技成长ETF（最多3只）
        selected_etfs.extend(etf_types['科技成长'][:3])
        
        # 优先级3：医药消费ETF（最多2只）
        selected_etfs.extend(etf_types['医药消费'][:2])
        
        # 优先级4：金融地产ETF（最多1只）
        selected_etfs.extend(etf_types['金融地产'][:1])
        
        # 优先级5：商品ETF（最多1只）
        selected_etfs.extend(etf_types['商品'][:1])
        
        # 如果还不够，从剩余ETF中补充
        all_codes = [etf['code'] for etf in liquid_etfs]
        remaining = [code for code in all_codes if code not in selected_etfs]
        
        # 去重
        selected_etfs = list(dict.fromkeys(selected_etfs))
        
        # 补充到目标数量
        if len(selected_etfs) < target_num and remaining:
            need = target_num - len(selected_etfs)
            selected_etfs.extend(remaining[:need])
        
        # 去重
        selected_etfs = list(dict.fromkeys(selected_etfs))
        
        # 截断到目标数量
        selected_etfs = selected_etfs[:target_num]
        
        # 输出筛选结果
        log.info(f"【ETF池】最终筛选出 {len(selected_etfs)} 只ETF:")
        for i, code in enumerate(selected_etfs, 1):
            # 获取ETF名称
            etf_info = next((etf for etf in liquid_etfs if etf['code'] == code), None)
            if etf_info:
                name_info = etf_info['name']
                avg_volume_info = etf_info.get('avg_volume', 0)  # 已经是万股单位
                market_cap_info = etf_info.get('market_cap', 0)
                if market_cap_info > 0:
                    log.info(f"  {i}. {code} - {name_info} (日均成交量:{avg_volume_info:.0f}万股, 规模:{market_cap_info:.1f}亿元)")
                else:
                    log.info(f"  {i}. {code} - {name_info} (日均成交量:{avg_volume_info:.0f}万股, 规模:未知)")
            else:
                log.info(f"  {i}. {code}")
...
class StrategyManager:
    ...
    def __init__(self):
        ...
        self.allocation_config = {
            'last_allocation_date': None,  # 上次资金分配日期
            'monthly_adjustment_day': 1,  # 每月几号调整资金
            'min_pool_ratio': 0.05,  # 最小资金池比例5%
            'max_pool_ratio': 0.95,  # 最大资金池比例95%
    ...
    def allocate_capital_pools(self, context):
        ...
        try:
            ...
            for strategy_id in self.strategies.keys():
                # 获取策略历史绩效数据
                performance_data = g.trade_stats.get('strategy_performance', {}).get(strategy_id, {})
                daily_returns = performance_data.get('daily_returns', [])
                
                # 如果数据不足（少于20天），使用初始比例
                if len(daily_returns) < 20:
                    initial_ratio = self.capital_pools.get(strategy_id, {}).get('ratio', 0.5)
                    strategy_scores[strategy_id] = initial_ratio
                    continue
                
                # 计算量化因子
                recent_returns = daily_returns[-20:]  # 最近20天数据
                ...
                # 综合评分（简单加权）
                score = (
                    avg_return * 0.4 +              # 收益率权重40%
                    sharpe_ratio * 0.3 +            # 夏普比率权重30%
                    (1 - abs(max_drawdown)) * 0.3   # 最大回撤权重30%
                )
                
                strategy_scores[strategy_id] = float(score)  # 显式转换为float
            ...
            if not strategy_scores or total_score <= 0:
                ...
                for strategy_id, score in strategy_scores.items():
                    raw_ratio = score / total_score
                    
                    # 应用5%-95%的限制
                    ratio = max(0.05, min(0.95, raw_ratio))
                    
                    # 资金池平滑过渡逻辑（与上一次比例的差值不超过20%）
                    last_ratio = self.capital_pools.get(strategy_id, {}).get('ratio', ratio)
                    max_change = 0.2
                    
                    if abs(ratio - last_ratio) > max_change:
                        if ratio > last_ratio:
                            ratio = last_ratio + max_change
                        else:
                            ratio = last_ratio - max_change
    ...
    def safe_order(self, context: Any, stock: str, amount: float, strategy_id: str, order_type: str = 'order', **kwargs) -> Dict[str, Any]:
        ...
        功能说明：
        1. 检查股票是否属于当前策略（避免策略间冲突）
        2. 记录详细买卖日志
        3. 更新策略标签系统
        4. 支持多种下单方式（order/order_value/order_target_value）
        5. 验证资金池是否充足
        
        参数:
            context: 策略上下文对象
            stock: 股票代码（如 '000001.XSHE'）
            amount: 下单数量或金额（根据order_type决定）
            strategy_id: 策略唯一标识符（如 'test_strategy_1', 'limit_up'）
            order_type: 下单方式，支持：
                - 'order': 按数量下单（amount为股数，正数买入，负数卖出）
                - 'order_value': 按金额下单（amount为金额，正数买入，负数卖出）
                - 'order_target_value': 目标市值下单（amount为目标市值）
                - 'order_target': 目标数量下单（amount为目标股数）
            **kwargs: 额外参数，传递给对应的聚宽下单函数
        ...
        使用示例:
            # 按数量买入100股
            result = g.strategy_manager.safe_order(context, '000001.XSHE', 100, 'test_strategy_1', 'order')
            
            # 按金额买入10000元
            result = g.strategy_manager.safe_order(context, '000001.XSHE', 10000, 'test_strategy_1', 'order_value')
            
            # 目标市值10000元（自动计算买卖数量）
            result = g.strategy_manager.safe_order(context, '000001.XSHE', 10000, 'test_strategy_1', 'order_target_value')
        ...
        try:
            ...
            # ========== 步骤3完成 ==========
            log.debug(f"[{strategy_id}] safe_order: 策略标签检查通过，继续执行 {action} 操作")
...
class BaseStrategy:
    ...
    def before_trading_start(self, context):
        ...
        try:
            # ========== 步骤1：检查策略是否启用 ==========
            if not self.enabled:
                log.debug(f"[{self.strategy_id}] 策略已禁用，跳过盘前准备")
                return
    ...
    def _update_strategy_positions(self, context):
        ...
        try:
            ...
            if hasattr(g, 'strategy_tags'):
                ...
                log.debug(f"[{self.strategy_id}] 当前持仓股票数: {len(self.positions)}")
                if self.positions:
                    log.debug(f"[{self.strategy_id}] 持仓股票: {self.positions}")
            else:
                self.positions = []
                log.warning(f"[{self.strategy_id}] 未找到全局策略标签系统 g.strategy_tags")
    ...
    def _update_capital_pool(self, context):
        ...
        try:
            ...
            if hasattr(g, 'strategy_manager') and hasattr(g.strategy_manager, 'capital_pools'):
                ...
                log.debug(f"[{self.strategy_id}] 策略资金池: {self.capital_pool:.2f}元")
            else:
                log.warning(f"[{self.strategy_id}] 未找到StrategyManager或资金池信息")
    ...
    def execute_trade(self, context):
        ...
        try:
            # ========== 步骤1：检查策略是否启用 ==========
            if not self.enabled:
                log.debug(f"[{self.strategy_id}] 策略已禁用，跳过交易逻辑")
                return
            
            # ========== 步骤2：检查当前交易时段 ==========
            time_status = get_trading_time_status(context)
            if time_status not in ['morning', 'afternoon']:
                log.debug(f"[{self.strategy_id}] 当前时段为 {time_status}，跳过交易逻辑")
                return
    ...
    def _prepare_for_trade(self, context):
        ...
        try:
            ...
            log.debug(f"[{self.strategy_id}] 交易前准备完成 - 资金池: {self.capital_pool:.2f}元, 持仓数: {len(self.positions)}")
    ...
    def _after_trade(self, context):
        ...
        try:
            ...
            log.debug(f"[{self.strategy_id}] 交易后处理完成 - 资金池: {self.capital_pool:.2f}元, 持仓数: {len(self.positions)}")
    ...
    def after_trading_end(self, context):
        ...
        try:
            # ========== 步骤1：检查策略是否启用 ==========
            if not self.enabled:
                log.debug(f"[{self.strategy_id}] 策略已禁用，跳过盘后处理")
                return
    ...
    def _record_daily_trades(self, context):
        ...
        try:
            ...
            if not valid_trades:
                log.debug(f"[{self.strategy_id}] 当日无交易记录")
                return
    ...
    def _cleanup_daily_temp(self):
        ...
        try:
            # 清理当日交易标记
            if hasattr(self, '_today_bought'):
                self._today_bought = False
            if hasattr(self, '_today_sold'):
                self._today_sold = False
            
            log.debug(f"[{self.strategy_id}] 当日临时标记已清理")
    ...
    def check_capital_sufficient(self, required_amount):
        ...
        try:
            ...
            if result['sufficient']:
                log.debug(f"[{self.strategy_id}] 资金池充足: 可用{current_capital:.2f}元, 需要{required_amount:.2f}元")
            else:
                log.warning(f"[{self.strategy_id}] 资金池不足: 可用{current_capital:.2f}元, 需要{required_amount:.2f}元, 缺口{shortage:.2f}元")
        ...
        try:
            ...
            if stock in g.strategy_tags:
                ...
                if owner_strategy != self.strategy_id:
                    ...
                    # 同一策略持有该股票，跳过
                    log.debug(f"[{self.strategy_id}] 股票 {stock} 已被当前策略持有，跳过标签添加")
                    return True
            
            # 添加策略标签
            g.strategy_tags[stock] = self.strategy_id
            log.debug(f"[{self.strategy_id}] 添加持仓标签: {stock} -> {self.strategy_id}")
            
            # 更新策略持仓列表
            if self.strategy_id not in g.strategy_positions:
                g.strategy_positions[self.strategy_id] = []
            if stock not in g.strategy_positions[self.strategy_id]:
                g.strategy_positions[self.strategy_id].append(stock)
                log.debug(f"[{self.strategy_id}] 更新持仓列表: {stock} 已添加到 {self.strategy_id}")
...
class ETFRotationStrategy(BaseStrategy):
    ...
    def before_trading_start(self, context):
        ...
        try:
            ...
            # ========== 自动获取ETF池（如果开启） ==========
            if self.params.get('auto_fetch_etf_pool', False):
                try:
                    # 获取配置参数
                    auto_fetch_num = self.params.get('auto_fetch_num', 10)
                    auto_fetch_min_days = self.params.get('auto_fetch_min_days', 180)
                    
                    log.info(f"[{self.strategy_id}] 开始自动获取ETF池...")
                    
                    # 调用ETF池获取函数
                    fetched_etf_pool = get_etf_pool_by_criteria(
                        context=context,
                        target_num=auto_fetch_num,
                        min_days_listed=auto_fetch_min_days
                    )
                    
                    # 如果获取成功，更新ETF池
                    if fetched_etf_pool and len(fetched_etf_pool) > 0:
                        self.params['etf_pool'] = fetched_etf_pool
                        log.info(f"[{self.strategy_id}] ETF池自动更新完成，共 {len(fetched_etf_pool)} 只ETF")
                        log.info(f"[{self.strategy_id}] ETF池详情: {fetched_etf_pool}")
                    else:
                        log.warning(f"[{self.strategy_id}] ETF池自动获取失败或返回空列表，使用预定义优质ETF池")
                        # 使用预定义的优质ETF池
                        predefined_etf_df = _get_predefined_etf_list()
                        if not predefined_etf_df.empty:
                            predefined_etf_pool = predefined_etf_df['code'].tolist()
                            self.params['etf_pool'] = predefined_etf_pool
                            log.info(f"[{self.strategy_id}] 已切换到预定义优质ETF池，共 {len(predefined_etf_pool)} 只ETF")
                
                except Exception as e:
                    log.error(f"[{self.strategy_id}] 自动获取ETF池失败: {str(e)}")
                    import traceback
                    log.error(traceback.format_exc())
                    # 出错时使用预定义的优质ETF池
                    log.info(f"[{self.strategy_id}] 使用预定义优质ETF池作为备选方案")
                    predefined_etf_df = _get_predefined_etf_list()
                    if not predefined_etf_df.empty:
                        predefined_etf_pool = predefined_etf_df['code'].tolist()
                        self.params['etf_pool'] = predefined_etf_pool
    ...
    def MOM(self, etf: str, context: Any) -> float:
        ...
        try:
            ...
            # 增加日志输出，方便调试
            log.debug(f"[{self.strategy_id}] {etf} 动量因子详情: "
                      f"年化收益率={annualized_returns:.4f}, "
                      f"判定系数={r_squared:.4f}, "
                      f"得分={score:.4f}")
...
class SwingStrategy(BaseStrategy):
    ...
    def execute_trade(self, context):
        ...
        try:
            ...
            if not self.handle_data_flag:
                log.debug(f"[{self.strategy_id}] 无法处理数据（handle_data_flag=False）")
                return
    ...
    def _is_first_trading_day_in_month(self, context):
        ...
        try:
            ...
            # 调试日志
            log.debug(f"[{self.strategy_id}] [_is_first_trading_day_in_month] 调试信息:")
            log.debug(f"  当前日期: {cur}")
            log.debug(f"  本月第一天: {month_start}")
            log.debug(f"  交易日列表: {list(days)}")
            log.debug(f"  交易日数量: {len(days)}")
            log.debug(f"  判断结果: {len(days) == 1 and days[0] == cur}")
    ...
    def buy_stocks(self, context: Any) -> None:
        ...
        # ========== 步骤1：检查是否可以处理数据 ==========
        if not self.handle_data_flag:
            log.debug(f"[{self.strategy_id}] 无法处理数据（handle_data_flag=False），跳过买入")
            return
    ...
    def sell_stocks(self, context: Any) -> None:
        ...
        # ========== 步骤1：检查是否可以处理数据 ==========
        if not self.handle_data_flag:
            log.debug(f"[{self.strategy_id}] 无法处理数据（handle_data_flag=False），跳过卖出")
            return
        ...
        for code, pos in list(context.portfolio.positions.items()):
            try:
                ...
                if hasattr(g, 'strategy_tags') and code in g.strategy_tags:
                    ...
                    if stock_strategy_id != self.strategy_id:
                        # 跳过不属于当前策略的持仓
                        log.debug(f"[{self.strategy_id}] 跳过其他策略的持仓: {code} (策略: {stock_strategy_id})")
                        continue
                else:
                ...
                if code == self.params['money_fund']:
                    # 冷静期期间：不卖出货币基金
                    if self.in_cooldown:
                        log.debug(f"[{self.strategy_id}] 冷静期内，不卖出货币基金ETF: {code}")
                        continue
                        ...
                        if self._is_last_trading_day_in_month(context):
                            if pos.total_amount > 0:
                                ...
                                if hasattr(g, 'strategy_tags') and code in g.strategy_tags:
                                    ...
                                    log.debug(f"[{self.strategy_id}] 删除货币基金策略标签: {code}")
                        continue
...
def get_current_data_safe(security_list=None):
    """
    安全获取实时数据（避免缓存问题）
    
    ⚠️ 重要：get_current_data()在同一定时任务中有缓存，无参数调用会缓存返回的股票列表
    解决方案：统一获取所有需要的数据，然后传递给子函数
    """
    try:
        data = get_current_data(security_list=security_list)
        return data if data is not None else {}
    except Exception as e:
        log.error(f"get_current_data_safe: 获取实时数据失败 - {str(e)}")
        return {}

# ============================================================================
# 测试用简单买卖逻辑（用于验证盘前盘后计算）
# ============================================================================
