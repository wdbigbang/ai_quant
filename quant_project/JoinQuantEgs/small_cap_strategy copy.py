...
def before_trading_start(context):
    ...
    try:
        ...
        if df is None or df.empty:
            g.handle_data_flag = False
            return
        log.info("获取市值数据行数: %d" % len(df))
        stock_pool = list(df['code'])
    except Exception as e:
        log.error('获取市值数据失败: {}'.format(e))
        g.handle_data_flag = False
        return
    ...
    if g.roe_filter:
        ...
        if not stock_pool:
            log.info('选股: 候选{}只 -> ROE筛选后0只'.format(roe_before))
            g.handle_data_flag = False
            return
    ...
    if g.roe_improve_filter:
        ...
        if not stock_pool:
            log.info('选股: 候选{}只 -> ROE改善后0只'.format(roe_after))
            g.handle_data_flag = False
            return
    ...
    try:
        ...
        log.info("最终选股数量: %d" % len(g.df2))
        g.handle_data_flag = True
        log.info('选股完成: 候选{}只 -> 最终{}只'.format(len(g.stock_list), len(g.df2)))
    except Exception as e:
...
def _filter_by_roe_improve(context, stock_list):
    ...
    try:
        ...
        return list(df.index)[:top_count]
    except Exception as e:
        log.warn('ROE改善筛选失败: {}'.format(e))
        return stock_list
