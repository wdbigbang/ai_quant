...
def _filter_by_roe(context, stock_list):
    ...
    try:
        ...
        for stock in debug_stocks:
            ...
            # 获取最近5个季度的净利润数据
            q_np = query(income.code, income.statDate, income.net_profit, income.pubDate) \
            ...
            # 获取最近5个季度的indicator.roe（用于对比）
            q_roe = query(indicator.code, indicator.statDate, indicator.roe) \
                .filter(indicator.code == stock)
            df_roe_hist = get_fundamentals(q_roe, g.current_date)
            
            if df_np is not None and not df_np.empty and df_roe_hist is not None and not df_roe_hist.empty:
                df_np = df_np.sort_values('statDate', ascending=False).head(5)
                df_eq = df_eq.sort_values('statDate', ascending=False).head(5) if df_eq is not None else None
                df_roe_hist = df_roe_hist.sort_values('statDate', ascending=False).head(5)
                ...
                for i, row in df_np.iterrows():
                    ...
                    if df_eq is not None and not df_eq.empty:
                        eq_match = df_eq[df_eq['statDate'] == stat_date]
                        if not eq_match.empty:
                            eq_val = eq_match.iloc[0]['total_owner_equities']
                    
                    # 查找同期的indicator.roe
                    roe_match = df_roe_hist[df_roe_hist['statDate'] == stat_date]
                    roe_hist = roe_match.iloc[0]['roe'] if not roe_match.empty else 0
                    
                    # 计算ROE
                    calc_roe = (np_val / eq_val * 100) if eq_val > 0 else 0
                    
                    log.info("  %s: 净利润=%.2f亿, 净资产=%.2f亿, 手动ROE=%.2f%%, indicator.roe=%.2f%%" % 
                             (stat_date, np_val/1e8, eq_val/1e8, calc_roe, roe_hist))
...
def _filter_by_roe_improve(context, stock_list):
    ...
    try:
        ...
        # 【新增调试】打印每只股票的原始数据（包含statDate）
        log.info("[聚宽调试] === ROE改善原始数据（前3只股票的完整数据）===")
        debug_codes = list(df['code'].unique())[:3]
        for code in debug_codes:
            code_df = df[df['code'] == code].sort_values('statDate', ascending=False) if 'statDate' in df.columns else df[df['code'] == code]
            log.info("  %s:" % code)
            for idx, row in code_df.head(5).iterrows():
                if 'statDate' in row:
                    log.info("    statDate=%s, ROE=%.4f" % (row['statDate'], row['roe']))
                else:
                    log.info("    ROE=%.4f" % row['roe'])
        log.info("[聚宽调试] ================================")
        
        # 【关键】增加002306的完整数据输出（用于对比）
        if '002306.XSHE' in df['code'].values:
            log.info("[聚宽关键] === 002306 完整ROE数据（改善值计算对照）===")
            code_306 = df[df['code'] == '002306.XSHE'].sort_values('statDate', ascending=False)
            log.info("  002306.XSHE:")
            for idx, row in code_306.iterrows():
                log.info("    statDate=%s, ROE=%.4f" % (row['statDate'], row['roe']))
            log.info("[聚宽关键] ================================")

        # 关键步骤：unstack（保存原始statDate用于验证列顺序）
        # 先保存第一只股票的statDate顺序，用于验证unstack后的列对应关系
        first_code = list(df['code'].unique())[0]
        first_code_statDates = df[df['code'] == first_code].sort_values('statDate', ascending=True)['statDate'].tolist()
        log.info("[聚宽关键] 第一只股票 %s 的statDate顺序（升序）: %s" % (first_code, str(first_code_statDates)))
        ...
        # 【关键】打印unstack后各列对应的实际季度（基于第一只股票验证）
        log.info("[聚宽调试] === unstack后各列含义（实际验证）===")
        for col_idx in range(min(5, len(df.columns))):
            log.info("  列%d (ROE_%d) = statDate=%s" % (col_idx, col_idx+1, first_code_statDates[col_idx] if col_idx < len(first_code_statDates) else "未知"))
        log.info("[聚宽调试] ========================")
        ...
        for code in list(df.index)[:5]:
            increase = df.loc[code, 'increase']
            roe1 = df.loc[code].iloc[0]
            roe2 = df.loc[code].iloc[1]
            roe3 = df.loc[code].iloc[2]
            roe4 = df.loc[code].iloc[3]
            roe5 = df.loc[code].iloc[4]
            log.info("  %s: 改善值=%.4f" % (code, increase))
            log.info("    ROE数据: ROE_1=%.2f, ROE_2=%.2f, ROE_3=%.2f, ROE_4=%.2f, ROE_5=%.2f" % 
                     (roe1, roe2, roe3, roe4, roe5))
            log.info("    计算过程: 4*%.2f - %.2f - %.2f - %.2f - %.2f = %.4f" % 
                     (roe5, roe1, roe2, roe3, roe4, increase))
        
        # 【关键】增加002306的改善值计算详细输出
        if '002306.XSHE' in df.index:
            log.info("[聚宽关键] === 002306 改善值计算详细过程 ===")
            roe1 = df.loc['002306.XSHE'].iloc[0]
            roe2 = df.loc['002306.XSHE'].iloc[1]
            roe3 = df.loc['002306.XSHE'].iloc[2]
            roe4 = df.loc['002306.XSHE'].iloc[3]
            roe5 = df.loc['002306.XSHE'].iloc[4]
            increase_306 = df.loc['002306.XSHE', 'increase']
            log.info("  002306.XSHE 改善值=%.4f" % increase_306)
            log.info("    ROE_1=%.2f (statDate=%s)" % (roe1, first_code_statDates[0]))
            log.info("    ROE_2=%.2f (statDate=%s)" % (roe2, first_code_statDates[1]))
            log.info("    ROE_3=%.2f (statDate=%s)" % (roe3, first_code_statDates[2]))
            log.info("    ROE_4=%.2f (statDate=%s)" % (roe4, first_code_statDates[3]))
            log.info("    ROE_5=%.2f (statDate=%s)" % (roe5, first_code_statDates[4]))
            log.info("    计算: 4*%.2f - %.2f - %.2f - %.2f - %.2f = %.4f" % (roe5, roe1, roe2, roe3, roe4, increase_306))
            log.info("[聚宽关键] ================================")
        
        # 【新增】打印改善值计算说明
        log.info("[聚宽调试] ========== 改善值计算说明 ==========")
        log.info("  公式: increase = 4*ROE_5 - ROE_1 - ROE_2 - ROE_3 - ROE_4")
        log.info("  含义: 最新季度权重最高(4倍)，减去前4个季度的ROE")
        log.info("  ROE_1=最早季度, ROE_5=最新季度")
        log.info("  改善值大 = 最新季度ROE显著高于前4季度平均水平")
        log.info("[聚宽调试] ====================================")
