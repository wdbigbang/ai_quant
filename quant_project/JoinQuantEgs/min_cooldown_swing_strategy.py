# 克隆自聚宽文章：https://www.joinquant.com/post/60233
# 标题：小市值日线交易波段加冷静期年收益57%
# 作者：wellfuture
...
def initialize(context):
    ...
    # 分钟风控与冷却期参数（可按需调整）
    g.uprate = 8.0      # 分钟级止盈阈值（%）
    g.downrate = -3.0   # 分钟级回补阈值（%）；置为 None 可关闭
    ...
    g.days_since_sell = 0
    g.money_fund = '511880.XSHG'  # 冷静期持有的货币基金ETF（可调整）
...
def before_trading_start(context):
    ...
    # 尝试取基本面，按流通市值排序（越小越靠前）
    try:
        q = query(valuation.code, valuation.circulating_market_cap, valuation.market_cap, valuation.pe_ratio, valuation.pe_ratio_lyr) \
        ...
        if df is None or df.empty:
            ...
            return
        # 过滤无效PE
        # df = df[(df['pe_ratio'] > 0) & (df['pe_ratio_lyr'] > 0)]
        # 按流通市值从小到大
...
def check_portfolio_decline(context):
    ...
    if decline_days >= need_num:
        ...
        log.info('连续{}天跌幅触发冷静期：阈值={:.2%}, 次数={}, 日期={}'.format(need_num, g.return_threshold, g.cooldown_count, d))
        return True
    return False
